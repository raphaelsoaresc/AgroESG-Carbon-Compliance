...
export default function Header({ carId, setCarId, onSearch, searchCount }: HeaderProps) {
  return (
    <header className="bg-gradient-to-br from-slate-900 via-green-950 to-slate-900 pb-32 pt-16 px-6 text-white shadow-2xl border-b-4 border-green-500">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 mb-16">
        ...
        <div className="flex-shrink-0 mt-4 md:mt-0">
          <Link 
            href="/caipora/planos" 
            className="bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold px-6 py-3 rounded-full transition-all uppercase tracking-widest border border-slate-700 shadow-lg inline-block"
          >
            Ver Planos
          </Link>
        </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="relative bg-white p-2 rounded-[2rem] shadow-2xl flex flex-col md:flex-row gap-2">
          ...
          />
          <button onClick={() => onSearch(carId)} className="bg-slate-900 hover:bg-black text-white px-12 py-6 rounded-2xl font-black text-xl transition-all active:scale-95">
            EXECUTAR AUDITORIA
          </button>
        ...
        <div className="flex justify-center gap-4 mt-8 flex-wrap">
          ...
          <button onClick={() => onSearch(DEMO_IDS[2])} className="group flex items-center gap-2 text-xs bg-green-500/10 hover:bg-green-500/20 text-green-200 px-6 py-3 rounded-full border border-green-500/30 transition-all backdrop-blur-sm">
