{{ config(
    materialized='view',
    schema='agro_esg_staging'
) }}

WITH source_data AS (
    SELECT * FROM {{ source('raw_data', 'ana_rios_app') }}
),

renamed_and_cleaned AS (
    SELECT
        -- Identificador: Código da Bacia (ANA)
        CAST(id_origem AS STRING) as basin_code,
        
        -- Atributos
        CAST(categoria AS STRING) as category,
        CAST(ordem AS INT64) as river_order,
        CAST(buffer_m AS INT64) as buffer_meters,
        
        -- Geometria WKT (EPSG:4326)
        CAST(wkt_geom_app AS STRING) as geometry_wkt,
        
        -- Auditoria
        CAST(file_hash AS STRING) as file_hash,
        CAST(ingested_at AS TIMESTAMP) as ingested_at

    FROM source_data
),

deduplicated AS (
    SELECT 
        *,
        ROW_NUMBER() OVER (
            PARTITION BY basin_code 
            ORDER BY ingested_at DESC
        ) as row_num
    FROM renamed_and_cleaned
)

SELECT 
    * EXCEPT(row_num)
FROM deduplicated
WHERE row_num = 1