...
final_status_calc AS (
    SELECT
        v.*,
        -- Cálculo do Alias (MD5) original para anonimização
        ...
        -- Status de Elegibilidade (Regras de Negócio Agro-Ambientais)
        CASE
            WHEN v.slave_labor_match_confidence = 'HIGH' THEN 'NOT ELIGIBLE - SOCIAL RISK (SLAVE LABOR)'
            
            -- Bloqueio por Status (Agora capturando CA e SU padronizados)
            ...
            WHEN v.mapbiomas_deforested_ha_raw > LEAST(v.noise_threshold, 0.1) AND (v.mapbiomas_date >= v.forest_code_date OR v.mapbiomas_date IS NULL) 
                 THEN 'NOT ELIGIBLE - DEFORESTATION (MAPBIOMAS)'
            
            -- REFINO CMN 5.081 (AMAZÔNIA) - Prioridade sobre embargo geral
            WHEN v.biome_name LIKE 'AMAZ%NIA' AND v.embargo_fixed_ha > 0.001 THEN 'NOT ELIGIBLE - AMZ EMBARGO (CMN 5.081)'
            ...
            -- Regra de Identidade e Outros Status (Resiliente a siglas AT/PE padronizadas)
        ...
        -- Cálculo do Passivo Financeiro Estimado (Soma de todas as multas)
        (
          COALESCE(v.defo_fixed_ha * v.fine_defo, 0) + 
          (CASE WHEN v.is_small_holder THEN 0 ELSE COALESCE(v.rl_deficit_ha * v.fine_rl, 0) END) + 
          (CASE WHEN v.is_settlement_identity OR v.is_traditional_identity OR v.is_quilombo_identity THEN 0 ELSE COALESCE(v.protected_area_fixed_ha * v.fine_protected, 0) END) + 
          COALESCE(v.embargo_fixed_ha * v.fine_embargo, 0) + 
          CASE WHEN v.app_deforested_ha_forensic > 0 THEN v.fine_app ELSE 0 END +
          CASE WHEN v.slave_labor_match_confidence IS NOT NULL THEN v.fine_slave ELSE 0 END
...
SELECT
    ...
    -- Geospatial Confidence (Thresholds Padronizados e Cruzamento MapBiomas)
    CASE 
    ...
    -- Áreas Protegidas (Removida redundância de protected_overlap_ha)
    ...
    (protected_area_fixed_ha > 0.01 AND NOT (is_settlement_identity OR is_traditional_identity OR is_quilombo_identity)) as is_protected_area_overlap,
    ...
    -- Passivos Financeiros Individuais
    ...
    ARRAY_TO_STRING(ARRAY(
        SELECT x FROM UNNEST([
            ...
            CASE WHEN (biome_name LIKE 'AMAZ%NIA' AND embargo_fixed_ha >= 0.001 AND (embargo_date >= forest_code_date OR embargo_date IS NULL)) THEN 'EMBARGO AMAZÔNIA (CMN 5.081)' END,
            CASE WHEN embargo_fixed_ha > 0.1 AND (embargo_date >= forest_code_date OR embargo_date IS NULL) THEN 'EMBARGO ATIVO' END,
    ...
    -- Evidência Técnica (Incluindo Alertas MapBiomas)
    CONCAT(
    ...
    -- Colunas Oficiais MapBiomas para Auditoria
    mapbiomas_alert_ids,
    ...
    -- Detalhamento Forense e Identidade
    forensic_ti_ha, forensic_quilombo_ha, forensic_uc_ha, forensic_settlement_ha, forensic_traditional_ha,
    is_settlement_identity, is_traditional_identity, is_quilombo_identity,
    max_slope_degrees, relief_classification,
    rl_status, rl_deficit_ha, rl_balance_ha,
