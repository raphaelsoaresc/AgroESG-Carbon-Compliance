...
CASE
        -- ==========================================================
        -- 1. BLOQUEIOS LEGAIS E SOCIAIS (HARD BLOCKS - NOT ELIGIBLE)
        -- Tudo que gera NOT ELIGIBLE deve vir primeiro para o teste de Embargo passar.
        -- ==========================================================
        WHEN v.slave_labor_match_confidence = 'HIGH' THEN 'NOT ELIGIBLE - SOCIAL RISK (SLAVE LABOR)'
        
        -- Status Críticos (Cancelado/Suspenso)
        WHEN UPPER(TRIM(v.registration_status)) IN ('CANCELADO', 'SUSPENSO', 'CA', 'C', 'SU', 'S') 
             OR UPPER(TRIM(v.registration_status_geometry)) IN ('CANCELADO', 'SUSPENSO', 'CA', 'C', 'SU', 'S') 
             THEN 'NOT ELIGIBLE - CAR STATUS'

        WHEN v.is_missing_geometry = TRUE THEN 'NOT ELIGIBLE - INVALID GEOMETRY'
        
        -- Sobreposições (Invasões)
        ...
        -- Desmatamento e APP (Com data confirmada)
        ...
        -- EMBARGOS (Subi para antes dos Manual Reviews para o teste passar)
        ...
        -- Se não bloqueou acima, mas tem dúvida, cai aqui.
        -- ==========================================================
        
        -- Status Pendente (Garante o teste de CAR Status)
        WHEN UPPER(TRIM(COALESCE(v.registration_status, ''))) IN ('PENDENTE', 'PE', 'P', 'EM ANALISE', 'ANALISE', 'A') 
             OR UPPER(TRIM(COALESCE(v.registration_status_geometry, ''))) IN ('PENDENTE', 'PE', 'P', 'EM ANALISE', 'ANALISE', 'A')
             THEN 'MANUAL_REVIEW_REQUIRED - PENDING STATE ANALYSIS'

        -- Desmatamento sem data (Vem depois do Embargo agora)
        WHEN v.mapbiomas_deforested_ha_raw > LEAST(v.noise_threshold, 0.1) 
             AND (v.mapbiomas_date IS NULL OR v.mapbiomas_date = '1900-01-01')
             THEN 'MANUAL_REVIEW - UNDATED DEFORESTATION'

        WHEN v.registration_status IS NULL THEN 'MANUAL_REVIEW_REQUIRED - MISSING STATUS DATA'
        ...
        -- Só chega aqui se não for Bloqueado nem Pendente.
        -- ==========================================================
...
fiscal_modules, city, city_data_source_origin, uf_origem, registration_status as car_status, solicitacao_adesao_pra,
